...
func startServer(hostAddr string, serviceType string, blockStoreAddr string) error {
	...
	if serviceType == "both" || serviceType == "meta" {
		metaStore := src.NewMetaStore(blockStoreAddr)
		src.RegisterMetaStoreServer(grpcServer, metaStore)
	}
	if serviceType == "both" || serviceType == "block" {
		blockStore := src.NewBlockStore()
		src.RegisterBlockStoreServer(grpcServer, blockStore)
	}
